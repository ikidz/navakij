...
var A__tune = 
{
	...
	node:
	{
		getOpacity:function(node)
		{
			...
			return parseFloat(node.style[A__tune.isGecko?'MozOpacity':'opacity'] || 1.0);
		},
	...
	event:
	{
		...
		{
			if ( A__tune.isIE && node.attachEvent )
			{
	...
	behavior:
	{
		...
		{
			if ( $notset(A__tune.__prop.page_scroll) )
			{
				A__tune.__prop.page_scroll = [$().s().overflow, $().ga('scroll')];
			}
...
A__tune.isGecko = !A__tune.isSafari&&!A__tune.isIE&&-1!=navigator.userAgent.indexOf('ecko');
A__tune.isSafari2 = A__tune.isSafari && -1 != navigator.appVersion.indexOf('Kit/4');
A__tune.isSafari3 = A__tune.isSafari && -1 != navigator.appVersion.indexOf('Kit/5');
...
ACNode.prototype.e = function(t, c, oneUseOnly)
{
	...
	//		console.log('*E* removing listeners for %s, listeners: %s', t, ser_listeners);
			...
			{
				A__tune.event.removeListener(this.$, t, self[ser_listeners[i]]);
...
//	console.log('*E* add listener self.%s for %s', hash, t);
	self[hash] = function(evt)
	{
		...
		{
			A__tune.event.removeListener(evt.$.$, t, self[hash]);
...
//				console.log('*E* stop self.%s for %s', hash, t);
	...
	A__tune.event.addListener(this.$, t, self[hash]);
	return this;
}
...
{
	if ( A__tune.isIE && this.$.attachEvent )
	{
...
ACNode.prototype.o = function(o)
{
	...
	{
		return A__tune.node.getOpacity(this.$);
	}
	A__tune.node.setOpacity(this.$, o);
	return this;
}
// *d*isplay
...
// `A_` converts any value into string - useful for numeric values before calling for String enhanced methods.
var A_ = function(t)
{
	return ''+t;
}
...
function __fry_gcnode_inner(inode)
{
	...
	for ( var i=0; i<num; i++ )
	{
		...
		if ( attr_name && 'fryse' == attr_name.substr(0,5) )
		{
			...
			{
				A__tune.event.removeListener(inode, type, self[listeners[ii]]);
				inode['on'+type] = null;
...
A__tune.event.addListener(self, 'load', function(evt)
{
...
A__tune.event.addListener(self, 'unload', function(evt)
{
	__gc_trash_node = $().$;
	__fry_gc_recycle();
});/*
...
fry.keyboard.initialize = function()
{
	...
	if (A__tune.isGecko)
	{
		react_as = 'ff_' + (A__tune.isMac ? 'mac' : 'win');
	}
	else if (A__tune.isSafari)
	{
		react_as = 'webkit';
	}
	else if (A__tune.isIE)
	{
		react_as = 'ie';
	}
	else if (A__tune.isOpera)
	{
		react_as = 'opera';
	}
	...
	document.onscroll = function(evt)
	{
		...
		{
			fry.keyboard.clipboard.node.style.top = (A__tune.isSafari? document.body.scrollTop : document.documentElement.scrollTop) + 'px';
		}
	}
	if (A__tune.isIE)
	{
	...
	code += "else { ";
	code += A__tune.isIE ? "if (86 == event.keyCode && event.ctrlKey){ fry.keyboard.clipboard.ie.node.value = ''; fry.keyboard.clipboard.ie.node.focus(); }\n" : "";
	code += "}\n";
	...
	if (A__tune.isGecko && A__tune.isMac)
	{
...
fry.keyboard.prepareClipboard = function()
{
	...
	fry.keyboard.clipboard.node = $().a($$('textarea')).w(20).h(20).pos(true).x(-2000).y(A__tune.isSafari? document.body.scrollTop : document.documentElement.scrollTop).$;
}
...
$(document.documentElement).e(A__tune.isSafari2?'mousedown':'click', function(evt)
{
	var elem = evt.$.$;
...
ac.chap.Window.prototype.setOptions = function(options)
{
	this.options = 
	{
		...
		font:{
			size:11,
			family: A__tune.isMac ? "Consolas, 'Bitstream Vera Sans mono', 'Courier', 'Monaco', monospaced" : "Consolas, 'Courier New', 'Courier', monospaced",
...
//	console.log('action_type:%s, params:%o', actionType, params);
	...
	switch ( actionType )
	{
		...
		case ac.chap.ACTION_SELECTION:
		{
			...
			{
				A__tune.behavior.clearSelection();
...
ac.chap.Window.prototype.captureEditAreaClick = function(evt, view)
{
	...
	var offset_y = evt.pageY - pos.y + (A__tune.isGecko ? view.nodeScrollArea.$.scrollTop : 0);
	var target  = evt.$.$;
	var used = false;
	...
	if ( null != pos )
	{
		...
		if ( 1 == this.window.state.caretPhase )
		{
			...
			if ( !skipScroll )
			{
				...
				else if ( node_row.offsetTop > this.options.rowHeight*(this.numRows-1)-A__tune.ui.scrollbarWidth )
				{
...
ac.chap.View.prototype.resize = function()
{
	...
	this.nodeFillArea.h(h-A__tune.ui.scrollbarWidth);
	$(this.nodeSelectionArea).h(h-A__tune.ui.scrollbarWidth+this.options.rowHeight);
...
ac.chap.View.prototype.recalculateNumCols = function(node, withoutScrollbar)
{
	...
	if (withoutScrollbar)
	{
		w -= A__tune.ui.scrollbarWidth+61;
	}
...
ac.chap.View.prototype.showInteractiveSearch = function()
{
	...
	var node = $().a($$()).pos(true).x(pos.x+58).y(pos.y).z(2000).w(this.nodeRoot.w()-A__tune.ui.scrollbarWidth-61).h(24).o(0.8);
...
//		console.log('OFFSET %s  ROWINDEX %s', offset, row_index);
	...
	h -= A__tune.ui.scrollbarWidth;
	w -= A__tune.ui.scrollbarWidth+3;
...
//		console.log('num visible for view :%s = %s, row_id.len = %s', this.index, this.numVisibleRows, this.window.row_id_map[this.index].length);
		...
		if ( this.nodeRoot.h()-A__tune.ui.scrollbarWidth > fill_area_h )
		{
			fill_area_h = this.nodeRoot.h()-A__tune.ui.scrollbarWidth;
		}
	...
	if ( parseInt(this.nodeSidebar.firstChild.style.top) != top_offset )
	{
		...
		this.nodeSidebar.firstChild.style.height  = (this.nodeSidebar.offsetHeight - A__tune.ui.scrollbarWidth - top_offset)+'px';		
	}
	this.renderSelection();
}
...
eamy.snippets.push(snippet);
snippet = {tab_activation: 'A_', code: 'A_SESSION['+"'"+'${1:variable}'+"'"+']'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'incl1', code: 'include_once '+"'"+'${1:file}'+"'"+';$0'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'A_', code: 'A_ENV['+"'"+'${1:variable}'+"'"+']'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'case', code: 'case '+"'"+'${1:variable}'+"'"+':\n	${0:# code...}\n	break;'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'else', code: 'else {\n	${0:# code...}\n}'};
...
snippet = {tab_activation: '/**', code: '/**\n * $0\n */'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'A_', code: 'A_REQUEST['+"'"+'${1:variable}'+"'"+']'};
eamy.snippets.push(snippet);
...
snippet = {tab_activation: 'switch', code: 'switch (${1:variable}) {\n	case '+"'"+'${2:value}'+"'"+':\n		${3:# code...}\n		break;\n	$0\n	default:\n		${4:# code...}\n		break;\n}'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'A_', code: 'A_POST['+"'"+'${1:variable}'+"'"+']'};
eamy.snippets.push(snippet);
...
snippet = {tab_activation: 'ret1', code: 'return true;$0'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'A_', code: 'A_GET['+"'"+'${1:variable}'+"'"+']'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'php', code: '?>$0<?php'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'A_', code: 'A_FILES['+"'"+'${1:variable}'+"'"+']'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'ifelse', code: '<?php if (${1:condition}): ?>\n	$2\n<?php else: ?>\n	$0\n<?php endif ?>'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'echoh', code: '<?php echo htmlentities(${1:$var}, ENT_QUOTES, '+"'"+'utf-8'+"'"+') ?>$0'};
...
snippet = {tab_activation: 'A_', code: '\A_FILES['+"'"+'${1:variable}'+"'"+']'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'A_', code: '\A_COOKIE['+"'"+'${1:variable}'+"'"+']'};
eamy.snippets.push(snippet);
snippet = {tab_activation: 'A_', code: '\A_SERVER['+"'"+'${1:variable}'+"'"+']'};
eamy.snippets.push(snippet);
...
eamy.snippets.push(snippet);
	...
	document.write('<link rel="stylesheet" href="assets/css/amy_style.css" type="text/css" media="screen" title="no title" charset="utf-8" />');
