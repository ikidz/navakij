...
class Print_tax extends CI_Model
{
    ...
    public function add_print_tax_many($tax_id, $title)
    {
        ...
        $this->fpdf->SetFont('fclamoon', 'B', 12);
        ...
        $this->fpdf->SetFont('fclamoon', 'B', 12);
        $this->fpdf->SetY(35);
        $this->fpdf->Cell(0, 5, setUTF8("บริษัท เท็ด อินโนเวชั่น จำกัด"), 0, 1);
        $this->fpdf->SetY(43);
        $this->fpdf->SetFont('fclamoon', 'B', 10);
        $this->fpdf->Cell(0, 5, setUTF8("33/4 อาคาร เดอะไนน์ ทาวเวอร์ แกรนด์ พระราม 9 ทาวเวอร์บี ชั้น 30 ห้อง ทีเอ็นบี 03 ถ. พระรามเก้า แขวง ห้วยขวาง เขต ห้วยขวาง กรุงเทพมหานคร"), 0, 1);
        $this->fpdf->SetFont('fclamoon', 'B', 12);
        $this->fpdf->SetY(29);
        $this->fpdf->SetX(132);
        $this->fpdf->SetFont('fclamoon', 'B', 18);
        $tax_no = "0105555001630";
            ...
            if ($tax['tax_type'] == 3) {
                if ($rate == 2 || $rate == 3 || $rate == 5) {
                    ...
                    if ($other == true) {
                        ...
                        $this->fpdf->Cell(30, 5, setUTF8(number_format(${'amt_tax_' . $rate}, 2)), 0, 0, 'R');
        ...
        $this->fpdf->Image('public/ted/sig-wannisa.png', 135, 255, 20, 20);
        $this->fpdf->SetY(266);
        $this->fpdf->SetX(117);
    ...
    public function add_print_tax($cheque_id, $title)
    {
        ...
        $this->fpdf->SetFont('fclamoon', 'B', 14);
        $this->fpdf->SetY(17);
        $this->fpdf->SetX(183);
        $this->fpdf->Cell(0, 5, setUTF8($tax['tax_docno']), 0, 1);
        $this->fpdf->SetX(183);
        ...
        $this->fpdf->SetFont('fclamoon', 'B', 12);
        $this->fpdf->SetY(35);
        $this->fpdf->Cell(0, 5, setUTF8("บริษัท กานดา ดิจิทัล จำกัด"), 0, 1);
        $this->fpdf->SetY(43);
        $this->fpdf->SetFont('fclamoon', 'B', 10);
        $this->fpdf->Cell(0, 5, setUTF8("เลขที่ 99 ชั้นที่ 3 ห้องเลขที่ L3-A03 ถนนรามคำแหง แขวงสวนหลวง เขตสวนหลวง กรุงเทพมหานคร"), 0, 1);
        $this->fpdf->SetFont('fclamoon', 'B', 12);
        $this->fpdf->SetY(29);
        $this->fpdf->SetX(132);
        $this->fpdf->SetFont('fclamoon', 'B', 18);
        $tax_no = "0105564027131";
        ...
        $this->fpdf->Image('public/ted/sig-wannisa.png', 135, 258, 20.125, 8.25);
        $this->fpdf->SetY(266);
        $this->fpdf->SetX(117);
        $this->fpdf->Cell(13, 5, setUTF8(date("d", strtotime($cheque['cheque_date']))), 0, 0, 'C');
        $this->fpdf->Cell(19, 5, setUTF8(thai_fullmonth($cheque['cheque_date'])), 0, 0, 'C');
