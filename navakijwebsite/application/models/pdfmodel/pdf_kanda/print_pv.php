...
class Print_pv extends CI_Model
{
    ...
    public function make($cheque_id)
    {
        $fontsize=12;
        $all_tax = 0;
        ...
        $this->fpdf->AddFont('fclamoon','','fclamoon.php');
	    $this->fpdf->AddFont('fclamoon','B','fclamoonb.php');
        $this->fpdf->AddPage('P', 'A4');

        
        $y = $this->fpdf->getY();
		$this->fpdf->Image('public/kanda/logo.png', 25,11,0,20);
		$this->fpdf->setY($y);
		$this->fpdf->SetFont('fclamoon','B',35);
		$this->fpdf->Cell( 180  , 15 , setUTF8('KANDA DIGITAL CO., LTD.') , 'TRL', 1 , 'C' );
		$this->fpdf->SetFont('fclamoon','B',25);
		$this->fpdf->Cell( 180  , 10 , setUTF8('ใบสำคัญจ่าย (PV.)') , 'BRL', 1 , 'C' );
        ...
        $this->fpdf->Cell(50, 8, setUTF8('PV.' . thai_shortyear($cheque['cheque_date']) . date("m", strtotime($cheque['cheque_date'])) . '/'), 'B', 1, 'L');
        $this->fpdf->Ln(5);
        ...
        foreach (explode('|', TAX_RATE) as $rate) {
            $ratekey = str_replace('.', '_', $rate);
            if ($cheque['cheque_tax' . $ratekey] > 0) {
                $all_tax += $cheque['cheque_tax' . $ratekey];
                $this->fpdf->SetFont('sukhumvitset', 'B', $fontsize);
                $this->fpdf->Cell(20, 7, setUTF8('หัก'), 'L', 0, 'R');
                $this->fpdf->SetFont('sukhumvitset', '', $fontsize);
                $this->fpdf->Cell(47, 7, setUTF8('ภาษี ณ ที่จ่าย  ' . $rate . '%'), 'R', 0, 'L');
                $this->fpdf->SetLineWidth(0.7);
                $this->fpdf->Cell(35, 7, number_format(${'sum_tax_' . $ratekey}, 2), 1, 0, 'C');
                $this->fpdf->SetLineWidth(0.50);
                $this->fpdf->Cell(33, 7, '', 'R', 0, 'L');
                $this->fpdf->Cell(45, 7, '(' . number_format($cheque['cheque_tax' . $ratekey], 2) . ')', 'R', 1, 'R');
            }
        }
        if ($all_tax > 0) {
            $this->fpdf->Cell(20, 8, '', 'L', 0);
            $this->fpdf->SetFont('sukhumvitset', 'B', 9);
            $this->fpdf->Cell(115, 7, setUTF8('รวม'), 'R', 0, 'L');
            $this->fpdf->Cell(45, 7, number_format($cheque['cheque_sum'] - $all_tax, 2), 'R', 1, 'R');
            $this->fpdf->SetFont('sukhumvitset', '', 9);
        }
        ...
        if ($cheque['pay_with'] == "cheque") {
            ...
            $this->fpdf->SetLineWidth(0.3);
            $this->fpdf->SetFont('sukhumvitset', 'B', 8);
            $this->fpdf->Cell(25, 7, setUTF8($cheque['bank_name']), 'TB', 0, 'L');
            $this->fpdf->SetFont('sukhumvitset', '', 8);
            ...
            $this->fpdf->SetLineWidth(0.3);
            $this->fpdf->SetFont('sukhumvitset', 'B', 8);
            $this->fpdf->Cell(35, 7, setUTF8($cheque['bookbank_branch']), 'TB', 0, 'L');
            $this->fpdf->SetFont('sukhumvitset', '', 8);
            $this->fpdf->SetTextColor(0, 0, 0);
            $this->fpdf->SetLineWidth(0.50);
        ...
        $this->fpdf->Image('public/ted/sig-wannisa.png', 28, 258, 22,22);
