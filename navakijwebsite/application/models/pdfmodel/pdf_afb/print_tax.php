...
class Print_tax extends CI_Model
{
    ...
    public function print_tax_batch_many($tax_id_start,$tax_id_end)
    {
        ...
        for($tax_id=$tax_id_start;$tax_id<=$tax_id_end;$tax_id++){
            ...
            if($row['cheque_pay_split']==1){
                ...
                $this->add_print_tax($tax['cheque_id'], 'ฉบับที่ 1 ( สำหรับผู้ถูกหักภาษี ณ ที่จ่าย ใช้แนบพร้อมกับแบบแสดงรายการภาษี )');
                $this->add_print_tax($tax['cheque_id'], 'ฉบับที่ 2 ( สำหรับผู้ถูกหักภาษี ณ ที่จ่าย เก็บไว้เป็นหลักฐาน)');
                $this->add_print_tax($tax['cheque_id'], 'ฉบับที่ 3 ( สำหรับผู้หักภาษี ณ ที่จ่าย ใช้แนบพร้อมกับแบบแสดงรายการภาษี )');
                $this->add_print_tax($tax['cheque_id'], 'ฉบับที่ 4 ( สำหรับผู้หักภาษี ณ ที่จ่าย เก็บไว้เป็นหลักฐาน)');
            }
        }
    ...
    public function add_print_tax($cheque_id, $title)
    {
        ...
        foreach (explode('|', TAX_RATE) as $rate) {
            ...
            $amt_tax += $cheque['cheque_tax' . $ratekey];
