  ...
  function cheque($cheque_id)
	{
		...
		switch($cheque['bank_id']){
			...
			break;
			default:
			 exit($cheque['bank_name'] . " ไม่รองรับการพิมพ์เช็ค");
		}
	}
	function tmb($cheque)
	{
...
//    	$this->fpdf->Text( $left_margin+7  , $top_margin+82 , setUTF8('A37 C31974473C011D0014A 0141074922C'));
	...
	function kbank($cheque)
	{
		...
		$left_margin=0;
   	$top_margin=0;
		// $this->fpdf->Image('public/cheque/kbank-cheque.png', $left_margin, $top_margin,178.31,0);
		// $this->fpdf->SetFont('cheque','',18);
   	// $this->fpdf->Text( $left_margin+7  , $top_margin+82 , setUTF8('A37 C31974473C011D0014A 0141074922C'));
	    ...
	    $years="{$years}";
	    $cheque_name = ($cheque['cheque_cash']=="1")?"เงินสด":$cheque['cheque_name'];
	    $this->fpdf->Cell( 111  , 6 , setUTF8('') , 0, 0 , 'L' );
	    $this->fpdf->Cell( 50  , 6 , setUTF8("{$day[0]}  {$day[1]}  {$month[0]}  {$month[1]}  {$years[0]}  {$years[1]}  {$years[2]}  {$years[3]}") , 0, 0 , 'L' );
		...
		if($cheque['cheque_cash']=="1"){	
			...
			$this->fpdf->SetY(5);
			$this->fpdf->SetX(42);
			$this->fpdf->Line(41,5,81,5);
			$this->fpdf->Line(41,12,81,12);
	    $this->fpdf->Cell( 10  , 6 , setUTF8('A/C PAYEE ONLY') , 0, 0 , 'L' );
			$this->fpdf->SetTextColor(0,0,0);
			$this->fpdf->SetDrawColor(0,0,0);
			for($ixx=20;$ixx<=28;$ixx++){
				$this->fpdf->Line(160,$ixx,172,$ixx-4);
				$ixx++;
			}
		}
	    ...
	    $this->fpdf->Ln();
	    $this->fpdf->SetY(20);
	    $this->fpdf->Cell( 20  , 6 , setUTF8('') , 0, 0 , 'L' );
	    $this->fpdf->Cell( 135  , 6 , setUTF8($cheque_name) , 0, 0 , 'L' );
	    $this->fpdf->Ln();
	    $this->fpdf->SetY(28);
	    $this->fpdf->Cell( 20  , 6 , setUTF8('') , 0, 0 , 'L' );
