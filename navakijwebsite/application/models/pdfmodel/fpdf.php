...
class FPDF extends CI_Model
{
	...
	function __construct($orientation = "P", $unit = "mm", $size = "A4")
	{
		...
		$this->StdPageSizes = [
			...
			"letter" => [612, 792],
			"legal" => [612, 1008],
			"tmb_cheque" => [502.6, 252.7],
			"kbank_cheque" => [253.7, 504.56],
			"kbank_payin" => [323.15, 595.276],
		];
	...
	{
		if ($this->header_ref) {
			eval($this->header_ref);
		}
	}
	function setFooter($ref)
	{
		$this->footer_ref = $ref;
	}
	function Footer()
	...
	function Output($name = "", $dest = "")
	{
		...
		if ($dest == "") {
			if ($name == "") {
				$name = $name ? basename($name) : "doc.pdf";
				$dest = "I";
			} else {
				$dest = "F";
			}
