...
class Managecategorymodel extends CI_Model {
    ...
    public function create(){
		...
		$uploadpath = realpath('').'/public/core/uploaded/category';
		...
		$file = $this->uploadmodel->do_upload($config, 'category_thumbnail', $_FILES['category_thumbnail']);
		/* Upload Desktop - End */
		...
		$validatedURL = validate_meta_url( $metaURL, 'categories', 'category_' );
        /* Generate META URL - End */

        $this->db->set('category_thumbnail', $file);
        $this->db->set('main_id', $this->input->post('main_id'));
        $this->db->set('is_use_icon', ( $this->input->post('is_use_icon') ? $this->input->post('is_use_icon') : 0 ));
        if( $this->input->post('is_use_icon') && $this->input->post('is_use_icon') == 1 ){
            // $this->db->set('category_icon', $this->input->post('category_icon'));
        }
    ...
    public function update(){
		...
		$file = $this->uploadmodel->edit_upload($config, 'category_thumbnail', $_FILES['category_thumbnail'], $info['category_thumbnail']);
		/* Upload Desktop - End */

        /* Generate META URL - Start */
        $metaURL = $this->input->post('category_meta_url');
		$validatedURL = validate_meta_url( $metaURL, 'categories', 'category_', $info['category_id'] );
        /* Generate META URL - End */
