		...
		<div class="widget-body form">
			...
			<form method="post" enctype="multipart/form-data" class="form-horizontal">
				...
				<div class="control-group">
					...
					<div class="controls">
						<input type="checkbox" name="is_use_icon" id="is_use_icon" value="1" <?php echo set_checkbox('is_use_icon', 1); ?> />
					</div>
				</div>
				...
				<div class="theicon control-group" style="display:none;">
					<label class="control-label" for="category_icon">ไอคอน : </label>
					<div class="controls">
						<select name="category_icon" id="category_icon" class="chosen" data-placeholder="เลือก Icon" style="font-family: 'FontAwesome',Tahoma;">
					       ...
					       <option  style="font-family: 'FontAwesome',Tahoma;" value="<?php echo $key; ?>" <?php if(set_value("category_icon")==$key){ ?> selected="selected" <?php } ?>><?php echo str_replace("\\","&#x",$text); ?> <?php echo ucfirst(str_replace("icon-","",$key)); ?></option>
					       <?php } ?>
						</select>
					</div>
				</div>
...
$(document).ready(function(){
	...
	/* input[name="is_use_icon"] handler - Start */
	$('body').on('click', 'input[name="is_use_icon"]', function(){
		if( $(this).prop('checked') === true ){
			$('.theicon').slideDown('fast');
			$('.cover').slideUp('fast');
		}else{
			$('.theicon').slideUp('fast');
			$('.cover').slideDown('fast');
		}
	});
	/* input[name="is_use_icon"] handler - End */
});
</script>