				...
				<div class="control-group">
					...
					<div class="controls">
						<div class="input-prepend">
						   ...
						   <input class="input-large" name="menu_label" id="username" type="text" placeholder="กรุณาระบุชื่อเมนู" value="<?php echo set_value('menu_label',$this->_data['row']['menu_label']); ?>" />	
						</div>
				...
				<div class="control-group">
					...
					<div class="controls">
						<select name="menu_link" id="menu_link" class="span4" data-placeholder="เลือกลิ้งค์เชื่อมโยง">
					       ...
					       <optgroup label="<?php echo strtoupper(str_replace("_"," ",$cont)); ?>">
					       		<?php foreach($rs as $method){ ?>
					       		<?php 
					       		
					       		$url =  $cont."/".$method; 
					       		?>
					       			<option value="<?php echo $url; ?>" <?php if(set_value("menu_link",$this->_data['row']['menu_link'])==$url){ ?> selected="selected" <?php } ?>><?php echo strtolower($url); ?></option>
					       		<?php } ?>
					       </optgroup>
					       <?php } ?>
						</select>
				...
				<div class="control-group">
					...
					<div class="controls">
						<select name="menu_icon" id="menu_icon" class="span4 chosen" data-placeholder="เลือก Icon" style="font-family: 'FontAwesome',Tahoma;">
					       ...
					       <option  style="font-family: 'FontAwesome',Tahoma;" value="<?php echo $key; ?>" <?php if(set_value("menu_icon",$this->_data['row']['menu_icon'])==$key){ ?> selected="selected" <?php } ?>><?php echo str_replace("\\","&#x",$text); ?> <?php echo ucfirst(str_replace("icon-","",$key)); ?></option>
					       <?php } ?>
						</select>
					</div>
				</div>
