<div class="span12">
	<div class="widget">
		...
		<div class="widget-body form">
			...
			<form method="get" action="<?php echo admin_url('manageiplogs/transactions/'.$info['hash']); ?>" enctype="multipart/form-data" id="sortform" class="form-horizontal form-inline">

				<div class="control-group">
					<label class="control-label" for="sort_keyword">คีย์เวิร์ด : </label>
					<div class="controls">
						<input type="text" class="span10" name="sort_keyword" id="sort_keyword" placeholder="ค้นหาจาก URL" value="<?php echo set_value('sort_keyword', $this->input->get('sort_keyword')); ?>" />
					</div>
				</div>
				...
				<div class="form-actions">
					<button type="submit" class="btn btn-primary" id="btn-submit"><i class="icon-search"></i> ค้นหา</button>
				 	<a class="btn" href="<?php echo admin_url("manageiplogs/transactions/".$info['hash']); ?>"><i class="icon-list"></i> ดูทั้งหมด</a>
				 	<button type="button" class="btn btn-success" id="btn-export"><i class="icon-download"></i> ส่งออกไปยัง Excels</button>
				 	<a class="btn btn-danger" href="<?php echo admin_url('manageiplogs/index'); ?>"><i class="icon-undo"></i> กลับไปยังหน้าหลัก</a>
				</div>
...
$(document).ready(function(){
	$('#btn-export').click(function(){
		$('#sortform').attr('action', '<?php echo admin_url('manageiplogs/exporttransactionlogs/'.$info['hash']); ?>');
		$('#sortform').submit();
