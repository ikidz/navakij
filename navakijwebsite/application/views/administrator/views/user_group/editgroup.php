				...
				<div class="control-group">
				 ...
				 <div class="controls">
				    <div class="input-prepend">
				       <span class="add-on"><i class="icon-group"></i></span><input class="input-medium" name="group_name" id="group_name" type="text" placeholder="กรุณาระบุชื่อกลุ่มผู้ใช้งาน" value="<?php echo set_value('group_name',$row['group_name']); ?>" />			 
				    </div>
				 </div>
				</div>
				<div class="control-group">
                                 ...
                                 <div class="controls">
                                    <select class="span3" data-placeholder="Choose a Company" tabindex="1" name="company_id">
                                       ...
                                       <option value="<?php echo $company['company_id']; ?>" <?php echo ($company['company_id']==set_value("company_id",$row['company_id']))?'selected="selected"':''; ?> ><?php echo $company['company_name']; ?></option>
                                       <?php } ?>
                                    </select>
                                 </div>
                              </div>
				<div class="control-group">
				 ...
				 <div class="controls">
				    <div class="superadmin-enable">
				       <input type="checkbox" class="toggle" <?php echo (set_value('group_superadmin',$row['group_superadmin'])=="yes")?'checked="checked"':""; ?> name="group_superadmin" id="group_superadmin" value="yes" />
				    </div>
				 </div>
				</div>
				<table class="table-menu table table-bordered table-advance <?php echo (set_value('group_superadmin',$row['group_superadmin'])=="yes")?'hidden':''; ?>">
					<thead>
						<tr>
							<th>ชื่อเมนู</th>
							<th width="50">อ่าน</th>
...
$(document).ready(function(){
	...
	/*$(".menu_handle").change(function(){
		var target = $(this).attr("data-set");
		var prev = $(this).attr("data-prev");
		var curr = $(this).attr("data-curr");
		if($(this).is(":checked")){
			$(target+".checkfirst").attr("checked",true);
		}else{
			$(target).attr("checked",false);
		}
	...
	var perm_array = <?php echo json_encode($this->admin_model->get_group_permision($row['group_id'])); ?>
	
	
	for(x in perm_array){
		if(perm_array[x]['read']=='true'){
			$("input[name='perm["+x+"][r]']").attr("checked",true);
		}else{
			$("input[name='perm["+x+"][r]']").attr("checked",false);
		}
		if(perm_array[x]['write']=='true'){
			$("input[name='perm["+x+"][w]']").attr("checked",true);
		}else{
			$("input[name='perm["+x+"][w]']").attr("checked",false);
		}
		if(perm_array[x]['delete']=='true'){
			$("input[name='perm["+x+"][d]']").attr("checked",true);
		}else{
			$("input[name='perm["+x+"][d]']").attr("checked",false);
		}
	}
	setTimeout(function(){
